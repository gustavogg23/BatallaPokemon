    ...
    public partial class BarraCarga : Form
    {
        ...
        private void CargarPokemones()
        {
            ...
            foreach (string linea in lineas)
            {
                ...
                int maxVida = int.Parse(datos[4]); // Se guarda la vida máxima del pokemon
                Pokemon pokemon = new Pokemon(int.Parse(datos[0]), datos[1], datos[2], datos[3], int.Parse(datos[4]), maxVida, int.Parse(datos[5]), int.Parse(datos[6]), int.Parse(datos[7]), int.Parse(datos[8]), int.Parse(datos[9]), "Ninguno", datos[10], datos[11], datos[12], ataques); // Se crea un objeto de la clase Pokemon con los datos de la línea
